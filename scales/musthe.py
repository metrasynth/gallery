...
class Note:
    ...
    * octave (default is 4).
    ...
    def __init__(self, note):
        ...
        self.note_id = \
        {'C': 0, 'D': 2, 'E': 4, 'F': 5, 'G': 7, 'A': 9, 'B': 11}[self.tone]
    ...
    def __add__(self, interval):
        ...
        if self.tone == 'A' and self.accidental.startswith(
                'b') and interval.number == 3 and interval.semitones == 3:
            new_note_tone = 'B'
        else:
...
class Chord:
    ...
    def __init__(self, root, chord_type='M'):
        ...
        if chord_type in self.chord_recipes.keys():
            ...
            raise Exception(
                'Invalid chord type supplied! current valid types: {} '.format(
    ...
    def __repr__(self):
        return "Chord(Note({!r}), {!r})".format(str(self.notes[0]),
                                                self.chord_type)
